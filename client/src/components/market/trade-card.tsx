...
/** Small segmented control shared by the metal and product rows. */
    ...
    <div className="grid gap-1.5" style={{ gridTemplateColumns: `repeat(${options.length}, minmax(0, 1fr))` }}>
      {options.map((opt) => (
        <Button
          ...
          className={cn("h-auto px-2 py-1.5 text-xs whitespace-normal", value === opt.key && SELECTED_GOLD)}
        >
          {opt.label}
        </Button>
      ))}
...
function BuyForm({ metal, product, onProductChange }: { metal: Metal; product: TradeProduct; onProductChange: (p: TradeProduct) => void }) {
  ...
  return (
    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-3.5">
      <ChipRow
        options={metalProducts.map((p) => ({ key: p.key, label: p.shortLabel }))}
        value={product.key}
        onChange={(key) => onProductChange(metalProducts.find((p) => p.key === key)!)}
      />
        ...
        <Tabs value={side} onValueChange={(v) => setSide(v as "buy" | "sell")}>
          ...
          <div className="mt-3 space-y-3.5">
            <div className="space-y-1.5">
              ...
              <ChipRow options={METALS.map((m) => ({ key: m, label: METAL_LABEL[m] }))} value={metal} onChange={onMetalChange} />
            </div>
