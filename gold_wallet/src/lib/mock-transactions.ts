...
/**
 * Stand-in wallet activity until a backend exists (this app is the frontend
 * on its own — there is no `/api/transactions` behind it), mirroring
 * mock-wallet.ts / mock-user.ts. Illustrative demo rows, not real trades: the
 * prices sit near mock-rates.ts's gold anchor (৳21,839/g) so the feed agrees
 * with the rates shown everywhere else.
 */
