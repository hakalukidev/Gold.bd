...
export const KARATS: Karat[] = ["22k", "21k", "18k", "sonaton"];

/** 1 bhori (ভরি), also called a vori or tola = 11.664 grams = 16 ana. */
export const GRAMS_PER_BHORI = 11.664;

// bajus.org publishes no JSON feed — this app scrapes the same public HTML
// page gold_wallet/server's own bajus.service.js does, independently, so
// this storefront has no runtime dependency on gold_wallet being up.
const BAJUS_URL = process.env.BAJUS_URL ?? "https://bajus.org/gold-price";
...
async function fetchBajusHtml(): Promise<string> {
  const res = await fetch(BAJUS_URL, {
...
function karatFromLabel(label: string): Karat | null {
  const numbered = label.match(/(\d+)\s*KARAT/i);
  if (numbered) return `${numbered[1]}k` as Karat;
 ...
 */
function parseReportDate(html: string): Date | null {
...
function extractTable(html: string, tableClass: string): string {
...
function extractRows(tableHtml: string, metal: Metal, reportedAt: Date | null, effectiveAt: Date): ScrapedRate[] {
...
function toRateRows(html: string): ScrapedRate[] {
...
export async function scrapeBajusRates(): Promise<ScrapedRate[]> {
  const html = await fetchBajusHtml();
  return toRateRows(html);
}
