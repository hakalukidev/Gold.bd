/**
 * Landing-page copy in both languages. Scoped to the marketing page
 * (src/app/page.tsx and src/components/landing/**) — the authenticated app
 * (dashboard, forms, admin) is English-only for now; extend this dictionary
 * and lib/i18n/use-t.ts's usages there if that's needed later.
 */

const dictionary = {
  bn: {
    nav: {
      tagline: "বিশ্বস্ত গোল্ড। খাঁটি মূল্য।",
      home: "হোম",
      aboutUs: "আমাদের সম্পর্কে",
      about: "সম্পর্কে",
      marketRate: "গোল্ড রেট",
      prices: "মূল্য",
      buyGold: "পণ্য",
      whyUs: "কেন আমরা",
      howItWorks: "কিভাবে কাজ করে",
      features: "সুবিধা",
      support: "সাপোর্ট",
      buyingGuide: "কেনার গাইড",
      faq: "সাধারণ জিজ্ঞাসা",
      calculator: "ক্যালকুলেটর",
      contactUs: "যোগাযোগ",
      login: "লগইন",
      register: "অ্যাকাউন্ট খুলুন",
      loginSignup: "লগইন / সাইন আপ",
      goldWallet: "গোল্ড ওয়ালেট | সাইন ইন",
      openMenu: "মেনু খুলুন",
      closeMenu: "মেনু বন্ধ করুন",
      switchToLight: "লাইট মোডে যান",
      switchToDark: "ডার্ক মোডে যান",
      cart: "কার্ট",
      cartEmpty: "আপনার কার্ট খালি।",
      cartRemove: "সরান",
      cartSubtotal: "সাবটোটাল",
      cartCheckout: "চেকআউটে যান",
    },
    statusStrip: {
      city: "ঢাকা",
      rateSource: "রেট সোর্স:",
    },
    hero: {
      eyebrow: "বাংলাদেশের বিশ্বস্ত",
      headingLine1: "গোল্ড সেলিং",
      headingLine2Before: "",
      headingLine2Gold: "প্ল্যাটফর্ম",
      headingLine2After: "",
      highlights: [
        { label: "১০০% খাঁটি গোল্ড" },
        { label: "বিশ্বস্ত ও নিরাপদ" },
        { label: "২৪/৭ কাস্টমার সাপোর্ট" },
      ],
      trustFeatures: [
        { label: "সারাদেশে ডেলিভারি" },
        { label: "২২ক্যা ও ২৪ক্যা হলমার্ক গোল্ড" },
        { label: "সহজ রিটার্ন পলিসি" },
        { label: "লক করা নিরাপদ পেমেন্ট" },
      ],
      ctaPrimary: "এখনই গোল্ড কিনুন",
      ctaSecondary: "আজকের রেট দেখুন",
      priceBadge: {
        label: "২২ ক্যারেট/গ্রাম",
      },
      rateCard: {
        liveLabel: "লাইভ গোল্ড রেট (৳)",
        label22k: "২২ক্যা",
        label21k: "২১ক্যা",
        perGram: "/ গ্রাম",
        updated: "এইমাত্র হালনাগাদ",
      },
      stats: [
        { value: "১০K+", label: "সন্তুষ্ট গ্রাহক" },
        { value: "৯৯.৯৯%", label: "বিশুদ্ধতার নিশ্চয়তা" },
        { value: "১০০%", label: "নিরাপদ পেমেন্ট" },
        { value: "২৪/৭", label: "কাস্টমার সাপোর্ট" },
      ],
    },
    why: {
      tabAsset: "কেন গোল্ড?",
      tabPlatform: "কেন Gold BD?",
      prevSlide: "আগেরটি",
      nextSlide: "পরেরটি",
      goToSlide: "স্লাইডে যান",
      asset: {
        heading: "কেন গোল্ডে সঞ্চয় করবেন?",
        intro: "নগদ টাকার মূল্য সময়ের সাথে কমে, কিন্তু সোনা ঐতিহাসিকভাবে সম্পদ সংরক্ষণের একটি নির্ভরযোগ্য মাধ্যম।",
        points: [
          { title: "মূল্যস্ফীতি প্রতিরোধী", description: "বাজার অস্থিতিশীল হলেও সোনা দীর্ঘমেয়াদে মূল্য ধরে রাখে।" },
          { title: "সহজে তারল্যযোগ্য", description: "যেকোনো সময় বাজার রেটে বিক্রি করে সাথে সাথে নগদ পেতে পারেন।" },
          { title: "ভগ্নাংশে কেনার সুযোগ", description: "পুরো ভরি না কিনে যত ইচ্ছা গ্রাম পরিমাণে শুরু করতে পারেন।" },
        ],
      },
      platform: {
        heading: "কেন Gold BD বেছে নেবেন?",
        intro: "প্রতিটি ফিচার তৈরি হয়েছে স্বচ্ছতা, নিরাপত্তা এবং সহজ ব্যবহারযোগ্যতাকে কেন্দ্র করে।",
        points: [
          { title: "লেজার-ব্যাকড লেনদেন", description: "প্রতিটি কেনা-বেচা ডাবল-এন্ট্রি লেজারে রেকর্ড হয়, সম্পূর্ণ অডিট ট্রেইলসহ।" },
          { title: "যাচাইকৃত KYC", description: "NID ভেরিফিকেশনের মাধ্যমে প্রতিটি অ্যাকাউন্ট নিরাপদ রাখা হয়।" },
          { title: "রিয়েল-টাইম রেট", description: "সবসময় হালনাগাদ রেট অনুযায়ী কেনাবেচা, কোনো লুকানো চার্জ ছাড়াই।" },
        ],
      },
    },
    todayPrice: {
      heading: "আজকের সোনার দাম",
      live: "লাইভ",
      updated: "হালনাগাদ",
      karat22: "২২ ক্যারেট",
      karat21: "২১ ক্যারেট",
      karat18: "১৮ ক্যারেট",
      sanatan: "সনাতন",
      perBhori: "/ ভরি",
      perGram: "/ গ্রাম",
      perAna: "/ আনা",
      perRoti: "/ রতি",
      perPoint: "/ পয়েন্ট",
      unitBhori: "ভরি",
      unitGram: "গ্রাম",
      unitTola: "তোলা",
      perTola: "/ তোলা",
      unitAna: "আনা",
      unitRoti: "রতি",
      unitPoint: "পয়েন্ট",
      today: "আজ",
      chartTitle: "গত কয়েক দিনের রেট",
      min: "সর্বনিম্ন",
      max: "সর্বোচ্চ",
      noHistory: "পর্যাপ্ত ডেটা নেই",
      unchanged: "অপরিবর্তিত",
      tagline: "বাংলাদেশের বৃহত্তম হলমার্কযুক্ত ও সনদপ্রাপ্ত ২২ ক্যারেট স্বর্ণ বার ও কয়েনের মার্কেটপ্লেস",
      miniChartLabel: "২২ ক্যারেট - প্রতি গ্রাম",
      basedOnRecords: "সর্বশেষ {count}টি রেকর্ড অনুসারে",
      headingGold: "আজকের সোনার দাম",
      headingSilver: "আজকের রুপার দাম",
      metalGold: "সোনা",
      metalSilver: "রুপা",
      selectMetal: "ধাতু নির্বাচন করুন",
    },
    featured: {
      kicker: "আমাদের কালেকশন",
      heading: "আমাদের পণ্য",
      subheading: "সবচেয়ে জনপ্রিয় গোল্ড ও সিলভার বার এবং কয়েন — হলমার্কযুক্ত বিশুদ্ধতায়, লাইভ রেটে।",
      viewAll: "সব বার ও কয়েন",
      buyNow: "এখনই কিনুন",
      addToCart: "কার্টে যোগ করুন",
      added: "যোগ হয়েছে ✓",
      vatIncluded: "ভ্যাট অন্তর্ভুক্ত।",
      withPremium: "প্রিমিয়ামসহ",
      perGram: "/গ্রাম",
      decreaseQty: "পরিমাণ কমান",
      increaseQty: "পরিমাণ বাড়ান",
      metalGold: "গোল্ড",
      metalSilver: "সিলভার",
      formBar: "বার",
      formCoin: "কয়েন",
      kickerBar: "মিন্টেড বার",
      kickerCoin: "সৃজিত কয়েন",
      karatBadge: "২২ ক্যারেট",
      weight0_5: "০.৫ গ্রাম",
      weight1: "১ গ্রাম",
      weight5: "৫ গ্রাম",
      weight10: "১০ গ্রাম",
    },
    antique: {
      heading: "প্রাচীন মুদ্রার সংগ্রহ",
      comingSoon: "শীঘ্রই আসছে",
      subheading: "প্রতিটি মুদ্রা এককভাবে গ্রেডকৃত ও সংগ্রাহক মূল্যে বিক্রি হবে — গলিত মূল্যে নয়।",
      notifyBody: "চালু হলে সবার আগে জানতে চান?",
      notifyCta: "অ্যাকাউন্ট খুলে অপেক্ষা করুন",
      items: {
        mughal: { era: "মুঘল আমল", title: "মুঘল স্বর্ণমুদ্রা", spec: "হাতে খোদাইকৃত · একমাত্র কপি" },
        colonial: { era: "ব্রিটিশ ভারত", title: "ঔপনিবেশিক যুগের মুদ্রা", spec: "সনদসহ যাচাইকৃত" },
        royal: { era: "রাজকীয় সংগ্রহ", title: "সীমিত সংস্করণ স্মারক মুদ্রা", spec: "সংখ্যাযুক্ত সিরিজ" },
      },
    },
    rateHistory: {
      trackerTitle: "লাইভ গোল্ড রেট ট্র্যাকার",
      trackerSubtitle: "কেনার আগে আপনার পরিমাণ হিসাব করে দেখুন — স্বচ্ছ রেট, কোনো লুকানো চার্জ নেই।",
      heading: "গোল্ড রেট ও ক্যালকুলেটর",
      subheading: "এই প্ল্যাটফর্মে অ্যাডমিন যে রেট সেট করেছেন তার প্রকৃত ইতিহাস — কোনো বাহ্যিক মার্কেট ডেটা নয়।",
      chartCardTitle: "সময়ের সাথে রেট",
      loading: "লোড হচ্ছে…",
      noData: "এখনো কোনো রেট সেট করা হয়নি।",
      singlePointLabel: "বর্তমান রেট",
      singlePointHint: "চার্ট দেখতে আরও রেট পয়েন্ট দরকার — অ্যাডমিন নতুন রেট সেট করলে এখানে যোগ হবে।",
      tableShow: "টেবিল হিসেবে দেখুন",
      tableHide: "টেবিল লুকান",
      tableDate: "তারিখ",
      tablePrice: "প্রতি গ্রাম",
      calcTitle: "গোল্ড ক্যালকুলেটর",
      calcCurrentRate: "বর্তমান রেট",
      calcPerGram: "/ গ্রাম",
      calcAmountLabel: "টাকার পরিমাণ (৳)",
      calcResultLabel: "আনুমানিক পরিমাণ",
      enterAmount: "টাকার পরিমাণ লিখুন",
      youWillGet: "আপনি পাবেন",
      livePrice: "লাইভ রেট",
      buyGold: "গোল্ড কিনুন",
      learnMore: "আরও জানুন",
    },
    features: {
      heading: "প্ল্যাটফর্মের সুবিধাসমূহ",
      subheading: "প্রতিটি ফিচার তৈরি হয়েছে স্বচ্ছতা ও নিরাপত্তাকে কেন্দ্র করে।",
      items: [
        { title: "লেজার-ব্যাকড লেনদেন", description: "প্রতিটি কেনা-বেচা ডাবল-এন্ট্রি লেজারে রেকর্ড হয় — কোনো ব্যালেন্স হারিয়ে যাওয়ার সুযোগ নেই।" },
        { title: "যাচাইকৃত KYC", description: "NID ভেরিফিকেশনের মাধ্যমে প্রতিটি অ্যাকাউন্ট নিরাপদ ও সুরক্ষিত রাখা হয়।" },
        { title: "রিয়েল-টাইম রেট", description: "সবসময় হালনাগাদ গোল্ড রেট অনুযায়ী কিনুন ও বিক্রি করুন, কোনো লুকানো চার্জ ছাড়াই।" },
        { title: "সার্বক্ষণিক ওয়ালেট অ্যাক্সেস", description: "নগদ ও গোল্ড ব্যালেন্স একসাথে দেখুন, যেকোনো সময় ডিপোজিট বা উইথড্র করুন।" },
      ],
    },
    howItWorks: {
      heading: "গোল্ড ওয়ালেট কীভাবে কাজ করে",
      subheading: "চারটি সহজ ধাপে সোনা কেনাবেচা শুরু করুন।",
      steps: [
        { step: "১", title: "অ্যাকাউন্ট খুলুন", description: "মোবাইল নম্বর ও পাসওয়ার্ড দিয়ে রেজিস্টার করুন, OTP দিয়ে ভেরিফাই করুন।" },
        { step: "২", title: "KYC সম্পন্ন করুন", description: "NID নম্বর ও প্রয়োজনীয় ডকুমেন্ট জমা দিন — যাচাই শেষে অ্যাকাউন্ট সক্রিয় হবে।" },
        { step: "৩", title: "ওয়ালেটে টাকা যোগ করুন", description: "ডিপোজিট করে ক্যাশ ব্যালেন্স রাখুন, যেকোনো সময় উইথড্র করতে পারবেন।" },
        { step: "৪", title: "কিনুন বা বিক্রি করুন", description: "লাইভ রেট দেখে গ্রাম হিসেবে সোনা কিনুন বা বিক্রি করুন — সাথে সাথে ওয়ালেট আপডেট হবে।" },
      ],
    },
    trust: {
      heading: "নিরাপত্তা যেভাবে নিশ্চিত করি",
      subheading: "কোনো তৃতীয় পক্ষের দাবি নয় — এই প্ল্যাটফর্মের নিরাপত্তা যেভাবে বাস্তবে কাজ করে।",
      badges: [
        { title: "ডাবল-এন্ট্রি লেজার", description: "প্রতিটি লেনদেন অডিটযোগ্য — কোনো ব্যালেন্স পরিবর্তন লেজারের বাইরে হয় না।" },
        { title: "এনক্রিপ্টেড সেশন", description: "পাসওয়ার্ড হ্যাশড ও httpOnly সেশন কুকির মাধ্যমে সুরক্ষিত।" },
        { title: "NID-ভিত্তিক KYC", description: "প্রতিটি অ্যাকাউন্ট যাচাই করা হয় জাতীয় পরিচয়পত্রের ভিত্তিতে।" },
        { title: "রেট-লিমিটেড API", description: "অস্বাভাবিক ট্রাফিক ও অপব্যবহার থেকে সুরক্ষা।" },
      ],
    },
    about: {
      heading: "Gold.bd সম্পর্কে",
      body: "Gold.bd তৈরি হয়েছে বাংলাদেশের প্রতিটি পরিবারের জন্য সোনা ও রুপা মালিকানাকে সহজ, স্বচ্ছ এবং সহজলভ্য করে তুলতে। আমরা হলমার্কযুক্ত, সনদপ্রাপ্ত মূল্যবান ধাতুর সাথে ব্যাংক-গ্রেড বিমাকৃত ভল্ট সংরক্ষণ এবং আধুনিক ডিজিটাল অভিজ্ঞতা একত্র করেছি — যাতে যে কেউ মাত্র ৫০০ টাকা দিয়েই শুরু করে প্রকৃত, বাস্তব সঞ্চয় গড়ে তুলতে পারেন।",
      stats: [
        { label: "গ্রাহক" },
        { label: "সংরক্ষিত ধাতু" },
        { label: "বিমাকৃত হোল্ডিং" },
      ],
    },
    tagline: {
      words: ["সহজ", "নিরাপদ", "স্বচ্ছ"],
      body: "প্রতিটি গ্রাম, প্রতিটি লেনদেন — লেজারে রেকর্ড করা, যেকোনো সময় যাচাইযোগ্য।",
    },
    faq: {
      heading: "সাধারণ জিজ্ঞাসা",
      subheading: "যা জানা দরকার শুরু করার আগে।",
      items: [
        { question: "সর্বনিম্ন কত গ্রাম সোনা কেনা যায়?", answer: "কোনো নির্দিষ্ট সর্বনিম্ন সীমা নেই — আপনার ওয়ালেটের ব্যালেন্স অনুযায়ী যেকোনো পরিমাণ গ্রামে কিনতে পারবেন।" },
        { question: "KYC ছাড়া কি ট্রেড করা যাবে?", answer: "না। কেনাবেচা শুরু করার আগে NID ভিত্তিক KYC ভেরিফিকেশন সম্পন্ন করতে হবে, যা /kyc পেজ থেকে জমা দেওয়া যায়।" },
        { question: "গোল্ড রেট কে নির্ধারণ করে?", answer: "অ্যাডমিন প্যানেল থেকে রেট আপডেট করা হয় এবং তা সাথে সাথে সব ইউজারের জন্য কার্যকর হয়।" },
        { question: "টাকা তোলা কতটা নিরাপদ?", answer: "প্রতিটি ডিপোজিট ও উইথড্র লেজার এন্ট্রি হিসেবে রেকর্ড হয়, তাই যেকোনো সময় লেনদেনের ইতিহাস যাচাই করা যায়।" },
      ],
    },
    footer: {
      tagline: "লেজার-ব্যাকড ডিজিটাল গোল্ড ওয়ালেট — স্বচ্ছ রেট, নিরাপদ লেনদেন।",
      companyHeading: "কোম্পানি",
      productHeading: "প্রোডাক্ট",
      supportHeading: "সমর্থন",
      policyHeading: "নীতিমালা",
      followUs: "আমাদের ফলো করুন",
      contactInfoHeading: "যোগাযোগের তথ্য",
      links: {
        home: "হোম",
        about: "সম্পর্কে",
        services: "সেবা এবং সুবিধা",
        news: "খবর এবং অন্তর্দৃষ্টি",
        charges: "চার্জ সময়সূচী",
        contact: "যোগাযোগ করুন",
        faq: "সাধারণ প্রশ্ন",
        pickupPoints: "পিক-আপ পয়েন্ট",
        careers: "ক্যারিয়ার",
        refundPolicy: "রিফান্ড এবং রিটার্ন পলিসি",
        deliveryPolicy: "ডেলিভারি পলিসি এবং প্রোটোকল",
        terms: "শর্তাবলী",
      },
      tradeLicenseLabel: "Trade License No",
      binLabel: "BIN",
      dbidLabel: "DBID No",
      phoneLabel: "হেল্পলাইন",
      emailLabel: "ই-মেইল",
      addressLabel: "নিবন্ধিত ঠিকানা",
      appHeading: "অ্যাপ ডাউনলোড করুন",
      appStoreEyebrow: "ডাউনলোড করুন",
      appStoreName: "App Store",
      googlePlayEyebrow: "এখনই পান",
      googlePlayName: "Google Play",
      copyright: "© {year} গোল্ড বিডি টেকনোলজিস লিমিটেড। সর্বস্বত্ব সংরক্ষিত।",
      developedBy: "ডেভেলপ করেছে",
    },
    ctaBand: {
      heading: "আজই আপনার গোল্ড ওয়ালেট খুলুন",
      body: "মাত্র কয়েক মিনিটে অ্যাকাউন্ট খুলুন এবং সোনা কেনাবেচা শুরু করুন।",
      cta: "শুরু করুন",
    },
    productsPage: {
      subheading: "গোল্ড বা সিলভার বার / কয়েন বেছে নিন — হলমার্কযুক্ত, সনদপ্রাপ্ত এবং বিমাকৃত।",
      hallmarkBadge: "হলমার্কযুক্ত ও সনদপ্রাপ্ত",
      trustInsured: "বিমাকৃত ভল্ট সংরক্ষণ অথবা হোম ডেলিভারি",
      trustCertificate: "সত্যতার সনদসহ",
      trustSellBack: "যেকোনো সময় লাইভ মার্কেট রেটে বিক্রি করুন",
    },
    checkoutPage: {
      heading: "চেকআউট",
      emptyTitle: "আপনার কার্ট খালি",
      emptyDescription: "কেনাকাটা শুরু করতে আমাদের পণ্য দেখুন।",
      emptyCta: "পণ্য দেখুন",
      addMore: "আরও যোগ করুন",
      deliveryHeading: "ডেলিভারি অপশন",
      homeTitle: "হোম ডেলিভারি",
      homeDescription: "শুধুমাত্র ঢাকা সিটি কর্পোরেশন এলাকার মধ্যে হোম ডেলিভারি",
      pickupTitle: "পিক-আপ",
      pickupDescription: "সারা বাংলাদেশের পিক-আপ পয়েন্ট থেকে নিজে উত্তোলন করুন",
      namePlaceholder: "নাম লিখুন",
      emailPlaceholder: "ই-মেইল লিখুন",
      recipientHeading: "প্রাপকের তথ্য",
      recipientNameLabel: "প্রাপকের নাম",
      recipientEmailLabel: "ই-মেইল",
      recipientPhoneLabel: "মোবাইল",
      addressLabel: "ঠিকানা",
      addressPlaceholder: "বাসা / রোড / এলাকা",
      divisionLabel: "বিভাগ",
      selectDivision: "বিভাগ নির্বাচন করুন",
      districtLabel: "জেলা",
      selectDistrict: "জেলা নির্বাচন করুন",
      noteLabel: "অতিরিক্ত বিবরণ (যদি থাকে)",
      notePlaceholder: "অতিরিক্ত বিবরণ লিখুন",
      orderHeading: "আপনার অর্ডার",
      quantityLabel: "পরিমাণ",
      subtotal: "সাবটোটাল",
      deliveryCharge: "ইনস্যুর্ড ডেলিভারি চার্জ",
      promoLabel: "প্রোমো কোড",
      promoPlaceholder: "প্রোমো কোড প্রবেশ করুন",
      promoApply: "প্রয়োগ করুন",
      promoComingSoon: "প্রোমো কোড শীঘ্রই আসছে",
      total: "মোট পরিশোধযোগ্য",
      paymentHeading: "পেমেন্ট পদ্ধতি নির্বাচন করুন",
      paymentBkash: "বিকাশ",
      paymentNagad: "নগদ",
      paymentCard: "ব্যাংক ট্রান্সফার",
      paymentOther: "এসএসএল কমার্স",
      placeOrder: "অর্ডার করুন",
      placingOrder: "পেমেন্ট গেটওয়েতে পাঠানো হচ্ছে…",
      verifyingPayment: "পেমেন্ট যাচাই করা হচ্ছে…",
      successTitle: "পেমেন্ট সফল হয়েছে",
      successDescription: "আপনার পেমেন্ট নিশ্চিত হয়েছে। আমাদের টিম শীঘ্রই ডেলিভারির বিষয়ে যোগাযোগ করবে।",
      successOrderNo: "অর্ডার নম্বর",
      failTitle: "পেমেন্ট ব্যর্থ হয়েছে",
      failDescription: "আপনার পেমেন্ট সম্পন্ন করা যায়নি। আপনার কার্ট অপরিবর্তিত রয়েছে — অনুগ্রহ করে আবার চেষ্টা করুন।",
      cancelTitle: "পেমেন্ট বাতিল করা হয়েছে",
      cancelDescription: "আপনি পেমেন্ট বাতিল করেছেন। আপনার কার্ট অপরিবর্তিত রয়েছে।",
      backToCheckout: "চেকআউটে ফিরে যান",
      continueShopping: "কেনাকাটা চালিয়ে যান",
      manualPayModalCancel: "বাতিল করুন",
      manualPaySubmit: "রিভিউর জন্য জমা দিন",
      manualPaySubmitting: "জমা দেওয়া হচ্ছে…",
      manualPayToLabel: "সঠিক পরিমাণ পাঠান এই নম্বরে",
      manualPayFetchError: "পেমেন্ট অ্যাকাউন্টের তথ্য লোড করা যায়নি। আবার চেষ্টা করুন।",
      bkashModalTitle: "বিকাশে পেমেন্ট করুন",
      bkashModalDescription: "নিচের নম্বরে টাকা পাঠান, তারপর যে নম্বর থেকে পাঠিয়েছেন এবং ট্রানজেকশন আইডি (TrxID) লিখুন।",
      nagadModalTitle: "নগদে পেমেন্ট করুন",
      nagadModalDescription: "নিচের নম্বরে টাকা পাঠান, তারপর যে নম্বর থেকে পাঠিয়েছেন এবং ট্রানজেকশন আইডি (TrxID) লিখুন।",
      senderNumberLabel: "আপনার বিকাশ/নগদ নম্বর",
      senderNumberPlaceholder: "01XXXXXXXXX",
      transactionIdLabel: "ট্রানজেকশন আইডি (TrxID)",
      transactionIdPlaceholder: "যেমন 8N7A6B5C4D",
      bankModalTitle: "ব্যাংক ট্রান্সফার",
      bankModalDescription: "নিচের অ্যাকাউন্টে জমা/ট্রান্সফার করুন, তারপর আপনার অ্যাকাউন্টের তথ্য দিন এবং পেমেন্টের প্রমাণসহ একটি ছবি যুক্ত করুন।",
      bankAccountNumberLabel: "আপনার ব্যাংক অ্যাকাউন্ট নম্বর",
      bankAccountNameLabel: "অ্যাকাউন্ট হোল্ডারের নাম",
      bankNameLabel: "আপনার ব্যাংকের নাম",
      bankBranchLabel: "শাখা",
      proofImageLabel: "পেমেন্টের প্রমাণ (স্ক্রিনশট/ছবি)",
      proofImageHint: "JPEG, PNG বা WebP, সর্বোচ্চ ৫MB",
      pendingHeading: "পেমেন্ট পর্যালোচনাধীন",
      pendingDescription: "আপনার পেমেন্ট যাচাই করা হচ্ছে। এতে কয়েক মিনিট থেকে কয়েক ঘণ্টা সময় লাগতে পারে — নিশ্চিত হলে আমরা আপনাকে এসএমএস পাঠাব।",
      pendingApprovedTitle: "পেমেন্ট নিশ্চিত হয়েছে",
      pendingApprovedDescription: "আপনার পেমেন্ট যাচাই করা হয়েছে। আমাদের টিম শীঘ্রই ডেলিভারির বিষয়ে যোগাযোগ করবে।",
      pendingDeclinedTitle: "পেমেন্ট যাচাই করা যায়নি",
      pendingDeclinedDescription: "আমরা আপনার পেমেন্ট যাচাই করতে পারিনি। অনুগ্রহ করে সাপোর্টে যোগাযোগ করুন।",
      pendingOrderNo: "অর্ডার নম্বর",
    },
    calculatorPage: {
      heading: "গোল্ড ক্যালকুলেটর",
      subheading: "গোল্ড, সিলভার, মেকিং চার্জ, ভরি-গ্রাম রূপান্তর ও যাকাত হিসাব — সবকিছু এক জায়গায়।",
      gold: {
        title: "গোল্ড ক্যালকুলেটর",
        description: "লাইভ রেট অনুযায়ী টাকার পরিমাণ থেকে গ্রাম হিসাব করুন।",
        amountLabel: "টাকার পরিমাণ (৳)",
        rateLabel: "বর্তমান রেট / গ্রাম",
        resultLabel: "আনুমানিক পরিমাণ",
      },
      silver: {
        title: "সিলভার ক্যালকুলেটর",
        description: "রূপার গ্রাম ও টাকার পরিমাণ রূপান্তর করুন।",
        amountLabel: "টাকার পরিমাণ (৳)",
        rateLabel: "রূপার রেট / গ্রাম (৳)",
        rateNote: "নির্দেশক রেট — প্রয়োজনে বদলে নিন, এটি লাইভ মার্কেট রেট নয়।",
        resultLabel: "আনুমানিক পরিমাণ",
      },
      makingCharge: {
        title: "মেকিং চার্জ ক্যালকুলেটর",
        description: "গোল্ডের ওজন ও মেকিং চার্জ শতাংশ দিয়ে মোট মূল্য হিসাব করুন।",
        weightLabel: "গোল্ডের ওজন (গ্রাম)",
        rateLabel: "রেট / গ্রাম (৳)",
        chargeLabel: "মেকিং চার্জ (%)",
        goldValueLabel: "গোল্ডের মূল্য",
        chargeAmountLabel: "মেকিং চার্জ",
        totalLabel: "মোট মূল্য",
      },
      bhoriGram: {
        title: "ভরি ↔ গ্রাম রূপান্তর",
        description: "১ ভরি = ১১.৬৬৩৮ গ্রাম — যেকোনো দিক থেকে রূপান্তর করুন।",
        bhoriLabel: "ভরি",
        gramLabel: "গ্রাম",
      },
      zakat: {
        title: "যাকাত ক্যালকুলেটর",
        description: "আপনার সোনার উপর যাকাত প্রযোজ্য কিনা এবং কত টাকা যাকাত দিতে হবে তা হিসাব করুন।",
        weightLabel: "সোনার ওজন (গ্রাম)",
        purityLabel: "বিশুদ্ধতা (ক্যারেট)",
        rateLabel: "বর্তমান রেট / গ্রাম (৳)",
        nisabNote: "নিসাব: ৮৭.৪৮ গ্রাম খাঁটি (২৪ক্যা) সোনার সমমূল্য।",
        eligible: "যাকাত প্রযোজ্য",
        notEligible: "যাকাত প্রযোজ্য নয় — মালিকানা নিসাবের নিচে।",
        marketValueLabel: "মোট বাজারমূল্য",
        zakatDueLabel: "প্রদেয় যাকাত (২.৫%)",
      },
    },
    buyingGuidePage: {
      heading: "গোল্ড কেনার গাইড",
      subheading: "নিরাপদ ও স্বচ্ছভাবে সোনা কেনার জন্য ধাপে ধাপে নির্দেশিকা।",
      steps: [
        { title: "অ্যাকাউন্ট তৈরি করুন", description: "মোবাইল নম্বর দিয়ে রেজিস্টার করে OTP দিয়ে ভেরিফাই করুন।" },
        { title: "KYC সম্পন্ন করুন", description: "NID নম্বর ও প্রয়োজনীয় ডকুমেন্ট জমা দিন — যাচাই শেষে অ্যাকাউন্ট সক্রিয় হবে।" },
        { title: "ওয়ালেটে টাকা যোগ করুন", description: "ডিপোজিট করে ক্যাশ ব্যালেন্স তৈরি করুন, যেকোনো সময় উইথড্র করতে পারবেন।" },
        { title: "ক্যালকুলেটর দিয়ে হিসাব করুন", description: "কেনার আগে লাইভ রেট দেখে, আপনার টাকায় কত গ্রাম সোনা পাবেন তা যাচাই করে নিন।" },
        { title: "গোল্ড কিনুন", description: "লাইভ রেটে গ্রাম হিসেবে সোনা কিনুন — সাথে সাথে ওয়ালেট আপডেট হবে।" },
        { title: "প্রয়োজনে বিক্রি করুন", description: "যেকোনো সময় বাজার রেটে বিক্রি করে সাথে সাথে নগদ টাকা তুলে নিতে পারবেন।" },
      ],
      tipsHeading: "কেনার আগে মনে রাখুন",
      tips: [
        "সবসময় লাইভ রেট যাচাই করে নিন — রেট প্রতি মুহূর্তে পরিবর্তিত হতে পারে।",
        "KYC ছাড়া কেনাবেচা সম্ভব নয়, তাই আগে থেকেই NID প্রস্তুত রাখুন।",
        "ছোট পরিমাণ দিয়ে শুরু করে ধীরে ধীরে বাড়াতে পারেন — কোনো নির্দিষ্ট সর্বনিম্ন সীমা নেই।",
        "প্রতিটি লেনদেন লেজারে রেকর্ড হয় — ট্রানজেকশন হিস্টোরি থেকে যেকোনো সময় যাচাই করুন।",
      ],
      ctaHeading: "শুরু করার জন্য প্রস্তুত?",
      ctaBody: "মাত্র কয়েক মিনিটে অ্যাকাউন্ট খুলুন এবং আজই প্রথম সোনা কিনুন।",
    },
  },
  en: {
    nav: {
      tagline: "Trusted Gold. Pure Value.",
      home: "Home",
      aboutUs: "About Us",
      about: "About",
      marketRate: "Gold Rate",
      prices: "Prices",
      buyGold: "Products",
      whyUs: "Why Us",
      howItWorks: "How it works",
      features: "Features",
      support: "Support",
      buyingGuide: "Buying Guide",
      faq: "FAQ",
      calculator: "Calculator",
      contactUs: "Contact Us",
      login: "Log in",
      register: "Create account",
      loginSignup: "Login / Sign Up",
      goldWallet: "Gold wallet | Sign in",
      openMenu: "Open menu",
      closeMenu: "Close menu",
      switchToLight: "Switch to light mode",
      switchToDark: "Switch to dark mode",
      cart: "Cart",
      cartEmpty: "Your cart is empty.",
      cartRemove: "Remove",
      cartSubtotal: "Subtotal",
      cartCheckout: "Go to checkout",
    },
    statusStrip: {
      city: "Dhaka",
      rateSource: "Rate source:",
    },
    hero: {
      eyebrow: "Bangladesh's Trusted ",
      headingLine1: "Gold Selling",
      headingLine2Before: "",
      headingLine2Gold: "Platform",
      headingLine2After: "",
      highlights: [
        { label: "100% Pure Gold" },
        { label: "Trusted & Secure" },
        { label: "24/7 Customer Support" },
      ],
      trustFeatures: [
        { label: "Countrywide Delivery" },
        { label: "22K & 24K Hallmark Gold" },
        { label: "Easy Return Policy" },
        { label: "Lock Safe & Secure Payment" },
      ],
      ctaPrimary: "Buy Gold Now",
      ctaSecondary: "Check Today's Gold Rate",
      priceBadge: {
        label: "22K/gram",
      },
      rateCard: {
        liveLabel: "Live Gold Rate (BDT)",
        label22k: "22K",
        label21k: "21K",
        perGram: "/ Gram",
        updated: "Updated just now",
      },
      stats: [
        { value: "10K+", label: "Happy Customers" },
        { value: "99.99%", label: "Purity Guaranteed" },
        { value: "100%", label: "Secure Payment" },
        { value: "24/7", label: "Customer Support" },
      ],
    },
    why: {
      tabAsset: "Why gold?",
      tabPlatform: "Why Gold BD?",
      prevSlide: "Previous",
      nextSlide: "Next",
      goToSlide: "Go to slide",
      asset: {
        heading: "Why save in gold?",
        intro: "Cash loses value over time, but gold has historically been a reliable way to preserve wealth.",
        points: [
          { title: "Inflation-resistant", description: "Gold holds its value over the long run even when markets are volatile." },
          { title: "Easy to liquidate", description: "Sell at the market rate any time and receive cash instantly." },
          { title: "Buy in fractions", description: "No need to buy a full bhori — start with as many grams as you like." },
        ],
      },
      platform: {
        heading: "Why choose Gold BD?",
        intro: "Every feature is built around transparency, security, and ease of use.",
        points: [
          { title: "Ledger-backed transactions", description: "Every trade is recorded as a double-entry ledger posting, with a complete audit trail." },
          { title: "Verified KYC", description: "Every account is secured through NID-based verification." },
          { title: "Real-time rate", description: "Trade at the always-current rate — no hidden fees." },
        ],
      },
    },
    todayPrice: {
      heading: "Today's Gold Price",
      live: "LIVE",
      updated: "Updated",
      karat22: "22 Karat",
      karat21: "21 Karat",
      karat18: "18 Karat",
      sanatan: "Traditional",
      perBhori: "/ bhori",
      perGram: "/ gram",
      perAna: "/ ana",
      perRoti: "/ roti",
      perPoint: "/ point",
      unitBhori: "Bhori",
      unitGram: "Gram",
      unitTola: "Tola",
      perTola: "/ tola",
      unitAna: "Ana",
      unitRoti: "Roti",
      unitPoint: "Point",
      today: "today",
      chartTitle: "Rate over the last few days",
      min: "Lowest",
      max: "Highest",
      noHistory: "Not enough data yet",
      unchanged: "Unchanged",
      tagline: "Bangladesh's largest hallmarked & certified 22-karat gold bar and coin marketplace",
      miniChartLabel: "22 Karat - per gram",
      basedOnRecords: "Based on the last {count} records",
      headingGold: "Today's Gold Price",
      headingSilver: "Today's Silver Price",
      metalGold: "Gold",
      metalSilver: "Silver",
      selectMetal: "Select metal",
    },
    featured: {
      kicker: "Our Collection",
      heading: "Our Products",
      subheading: "Our best-selling gold & silver bars and coins — hallmark purity, priced at the live rate.",
      viewAll: "All bars & coins",
      buyNow: "Buy now",
      addToCart: "Add to cart",
      added: "Added ✓",
      vatIncluded: "VAT included.",
      withPremium: "incl. premium",
      perGram: "/gram",
      decreaseQty: "Decrease quantity",
      increaseQty: "Increase quantity",
      metalGold: "Gold",
      metalSilver: "Silver",
      formBar: "Bar",
      formCoin: "Coin",
      kickerBar: "Minted Bar",
      kickerCoin: "Struck Coin",
      karatBadge: "22 Karat",
      weight0_5: "0.5 Gram",
      weight1: "1 Gram",
      weight5: "5 Gram",
      weight10: "10 Gram",
    },
    antique: {
      heading: "Antique Coin Collection",
      comingSoon: "Coming soon",
      subheading: "Every coin individually graded and sold at collector value — not melt value.",
      notifyBody: "Want to know the moment it launches?",
      notifyCta: "Create an account & wait for it",
      items: {
        mughal: { era: "Mughal era", title: "Mughal gold coin", spec: "Hand-engraved · one of a kind" },
        colonial: { era: "British India", title: "Colonial-era coin", spec: "Certified & verified" },
        royal: { era: "Royal collection", title: "Limited edition commemorative", spec: "Numbered series" },
      },
    },
    rateHistory: {
      trackerTitle: "LIVE Gold Rate Tracker",
      trackerSubtitle: "Work out your purchase before you buy — a transparent rate, no hidden fees.",
      heading: "Gold rate & calculator",
      subheading: "This is the actual history of rates this platform's admin has set — not external market data.",
      chartCardTitle: "Rate over time",
      loading: "Loading…",
      noData: "No rate has been set yet.",
      singlePointLabel: "Current rate",
      singlePointHint: "A chart needs a few more rate points — it'll appear here as the admin sets new rates.",
      tableShow: "View as table",
      tableHide: "Hide table",
      tableDate: "Date",
      tablePrice: "Per gram",
      calcTitle: "Gold calculator",
      calcCurrentRate: "Current rate",
      calcPerGram: "/ gram",
      calcAmountLabel: "Amount (৳)",
      calcResultLabel: "Estimated amount",
      enterAmount: "Enter amount",
      youWillGet: "You will get",
      livePrice: "Live price",
      buyGold: "Buy gold",
      learnMore: "Learn more",
    },
    features: {
      heading: "Platform features",
      subheading: "Every feature is built around transparency and security.",
      items: [
        { title: "Ledger-backed transactions", description: "Every trade is recorded on a double-entry ledger — no balance can ever go missing." },
        { title: "Verified KYC", description: "Every account is kept safe and secure through NID verification." },
        { title: "Real-time rate", description: "Buy and sell at the always-current gold rate, with no hidden fees." },
        { title: "Wallet, anytime", description: "See your cash and gold balances together, and deposit or withdraw whenever you like." },
      ],
    },
    howItWorks: {
      heading: "How Gold Wallet works",
      subheading: "Start trading gold in four simple steps.",
      steps: [
        { step: "1", title: "Create an account", description: "Register with your mobile number and password, then verify with an OTP." },
        { step: "2", title: "Complete KYC", description: "Submit your NID number and the required documents — your account activates once verified." },
        { step: "3", title: "Fund your wallet", description: "Deposit to build a cash balance, and withdraw whenever you need to." },
        { step: "4", title: "Buy or sell", description: "Trade gold by the gram at the live rate — your wallet updates instantly." },
      ],
    },
    trust: {
      heading: "How security actually works here",
      subheading: "Not a third-party claim — this is how this platform's security actually works.",
      badges: [
        { title: "Double-entry ledger", description: "Every transaction is auditable — no balance ever changes outside the ledger." },
        { title: "Encrypted sessions", description: "Passwords are hashed, and sessions are secured with httpOnly cookies." },
        { title: "NID-based KYC", description: "Every account is verified against a national ID." },
        { title: "Rate-limited API", description: "Protection against abusive or abnormal traffic." },
      ],
    },
    about: {
      heading: "About Gold.bd",
      body: "Gold.bd was built to make gold and silver ownership simple, transparent, and accessible for every household in Bangladesh. We combine hallmarked, certified precious metals with bank-grade insured vaulting and a modern digital experience — so anyone can start building real, tangible savings from as little as 500 BDT.",
      stats: [
        { label: "Customers" },
        { label: "Metal vaulted" },
        { label: "Insured holdings" },
      ],
    },
    tagline: {
      words: ["Easy", "Secure", "Transparent"],
      body: "Every gram, every transaction — recorded on the ledger, verifiable any time.",
    },
    faq: {
      heading: "Frequently asked questions",
      subheading: "What you need to know before you start.",
      items: [
        { question: "What's the minimum amount of gold I can buy?", answer: "There's no fixed minimum — you can buy any amount in grams your wallet balance allows." },
        { question: "Can I trade without KYC?", answer: "No. NID-based KYC verification is required before trading, submitted from the /kyc page." },
        { question: "Who sets the gold rate?", answer: "An admin updates the rate from the admin panel, and it takes effect for every user immediately." },
        { question: "How safe is withdrawing money?", answer: "Every deposit and withdrawal is recorded as a ledger entry, so transaction history can be verified at any time." },
      ],
    },
    footer: {
      tagline: "A ledger-backed digital gold wallet — transparent rates, secure transactions.",
      companyHeading: "Company",
      productHeading: "Product",
      supportHeading: "Support",
      policyHeading: "Policies",
      followUs: "Follow us",
      contactInfoHeading: "Contact info",
      links: {
        home: "Home",
        about: "About",
        services: "Services & benefits",
        news: "News & insights",
        charges: "Charge schedule",
        contact: "Contact us",
        faq: "FAQ",
        pickupPoints: "Pick-up points",
        careers: "Careers",
        refundPolicy: "Refund & return policy",
        deliveryPolicy: "Delivery policy & protocol",
        terms: "Terms & conditions",
      },
      tradeLicenseLabel: "Trade License No",
      binLabel: "BIN",
      dbidLabel: "DBID No",
      phoneLabel: "Helpline",
      emailLabel: "Email",
      addressLabel: "Registered address",
      appHeading: "Download the app",
      appStoreEyebrow: "Download on the",
      appStoreName: "App Store",
      googlePlayEyebrow: "Get it on",
      googlePlayName: "Google Play",
      copyright: "© {year} Gold BD Technologies Limited. All rights reserved.",
      developedBy: "Developed by",
    },
    ctaBand: {
      heading: "Open your gold wallet today",
      body: "Create an account in a few minutes and start trading gold.",
      cta: "Get started",
    },
    productsPage: {
      subheading: "Select a gold or silver bar / coin — hallmarked, certified, and insured.",
      hallmarkBadge: "Hallmarked & certified",
      trustInsured: "Insured vault storage or home delivery",
      trustCertificate: "Certificate of authenticity included",
      trustSellBack: "Sell back anytime at live market rate",
    },
    checkoutPage: {
      heading: "Checkout",
      emptyTitle: "Your cart is empty",
      emptyDescription: "Browse our products to start shopping.",
      emptyCta: "Browse products",
      addMore: "Add more",
      deliveryHeading: "Delivery options",
      homeTitle: "Home delivery",
      homeDescription: "Home delivery within Dhaka City Corporation area only",
      pickupTitle: "Pick-up",
      pickupDescription: "Collect yourself from a pick-up point anywhere in Bangladesh",
      namePlaceholder: "Enter name",
      emailPlaceholder: "Enter email",
      recipientHeading: "Recipient information",
      recipientNameLabel: "Recipient's name",
      recipientEmailLabel: "Email",
      recipientPhoneLabel: "Mobile",
      addressLabel: "Address line",
      addressPlaceholder: "House / road / area",
      divisionLabel: "Division",
      selectDivision: "Select division",
      districtLabel: "District",
      selectDistrict: "Select district",
      noteLabel: "Additional notes (optional)",
      notePlaceholder: "Add any additional notes",
      orderHeading: "Your order",
      quantityLabel: "Quantity",
      subtotal: "Subtotal",
      deliveryCharge: "Insured delivery charge",
      promoLabel: "Promo code",
      promoPlaceholder: "Enter promo code",
      promoApply: "Apply",
      promoComingSoon: "Promo codes are coming soon",
      total: "Total payable",
      paymentHeading: "Select payment method",
      paymentBkash: "bKash",
      paymentNagad: "Nagad",
      paymentCard: "Bank Transfer",
      paymentOther: "SSL Commerz",
      placeOrder: "Place order",
      placingOrder: "Redirecting to payment…",
      verifyingPayment: "Verifying your payment…",
      successTitle: "Payment successful",
      successDescription: "Your payment is confirmed. Our team will contact you shortly about delivery.",
      successOrderNo: "Order number",
      failTitle: "Payment failed",
      failDescription: "We couldn't complete your payment. Your cart is untouched — please try again.",
      cancelTitle: "Payment cancelled",
      cancelDescription: "You cancelled the payment. Your cart is untouched.",
      backToCheckout: "Back to checkout",
      continueShopping: "Continue shopping",
      manualPayModalCancel: "Cancel",
      manualPaySubmit: "Submit for review",
      manualPaySubmitting: "Submitting…",
      manualPayToLabel: "Send the exact amount to",
      manualPayFetchError: "Could not load payment account details. Please try again.",
      bkashModalTitle: "Pay with bKash",
      bkashModalDescription: "Send money to the number below, then enter the number you sent from and the transaction ID (TrxID).",
      nagadModalTitle: "Pay with Nagad",
      nagadModalDescription: "Send money to the number below, then enter the number you sent from and the transaction ID (TrxID).",
      senderNumberLabel: "Your bKash/Nagad number",
      senderNumberPlaceholder: "01XXXXXXXXX",
      transactionIdLabel: "Transaction ID (TrxID)",
      transactionIdPlaceholder: "e.g. 8N7A6B5C4D",
      bankModalTitle: "Bank transfer",
      bankModalDescription: "Deposit or transfer to the account below, then fill in your account details and attach a photo of the payment proof.",
      bankAccountNumberLabel: "Your bank account number",
      bankAccountNameLabel: "Account holder's name",
      bankNameLabel: "Your bank's name",
      bankBranchLabel: "Branch",
      proofImageLabel: "Payment proof (screenshot/photo)",
      proofImageHint: "JPEG, PNG or WebP, up to 5MB",
      pendingHeading: "Payment under review",
      pendingDescription: "Your payment is being checked. This can take a few minutes to a few hours — we'll send you an SMS once it's confirmed.",
      pendingApprovedTitle: "Payment confirmed",
      pendingApprovedDescription: "Your payment has been verified. Our team will contact you shortly about delivery.",
      pendingDeclinedTitle: "Payment could not be verified",
      pendingDeclinedDescription: "We couldn't verify your payment. Please contact support.",
      pendingOrderNo: "Order number",
    },
    calculatorPage: {
      heading: "Gold calculator",
      subheading: "Gold, silver, making charge, bhori-gram conversion, and zakat — all in one place.",
      gold: {
        title: "Gold calculator",
        description: "Work out how many grams your money buys at the live rate.",
        amountLabel: "Amount (৳)",
        rateLabel: "Current rate / gram",
        resultLabel: "Estimated amount",
      },
      silver: {
        title: "Silver calculator",
        description: "Convert between silver grams and amount.",
        amountLabel: "Amount (৳)",
        rateLabel: "Silver rate / gram (৳)",
        rateNote: "Indicative rate — edit it as needed, this isn't a live market feed.",
        resultLabel: "Estimated amount",
      },
      makingCharge: {
        title: "Making charge calculator",
        description: "Work out the total price from gold weight and a making charge percentage.",
        weightLabel: "Gold weight (grams)",
        rateLabel: "Rate / gram (৳)",
        chargeLabel: "Making charge (%)",
        goldValueLabel: "Gold value",
        chargeAmountLabel: "Making charge",
        totalLabel: "Total price",
      },
      bhoriGram: {
        title: "Bhori ↔ Gram converter",
        description: "1 bhori = 11.6638 grams — convert either direction.",
        bhoriLabel: "Bhori",
        gramLabel: "Gram",
      },
      zakat: {
        title: "Zakat calculator",
        description: "Check whether zakat applies to your gold, and how much is due.",
        weightLabel: "Gold weight (grams)",
        purityLabel: "Purity (karat)",
        rateLabel: "Current rate / gram (৳)",
        nisabNote: "Nisab: the value of 87.48 grams of pure (24K) gold.",
        eligible: "Zakat applies",
        notEligible: "Zakat doesn't apply — below the nisab threshold.",
        marketValueLabel: "Total market value",
        zakatDueLabel: "Zakat due (2.5%)",
      },
    },
    buyingGuidePage: {
      heading: "Gold buying guide",
      subheading: "A step-by-step guide to buying gold safely and transparently.",
      steps: [
        { title: "Create an account", description: "Register with your mobile number, then verify with an OTP." },
        { title: "Complete KYC", description: "Submit your NID number and the required documents — your account activates once verified." },
        { title: "Fund your wallet", description: "Deposit to build a cash balance, and withdraw whenever you need to." },
        { title: "Run the numbers", description: "Before you buy, check the live rate and use the calculator to see how many grams your money buys." },
        { title: "Buy gold", description: "Trade gold by the gram at the live rate — your wallet updates instantly." },
        { title: "Sell whenever you need to", description: "Sell at the market rate any time and receive cash instantly." },
      ],
      tipsHeading: "Before you buy, keep in mind",
      tips: [
        "Always check the live rate — it can change from moment to moment.",
        "Trading requires KYC, so have your NID ready ahead of time.",
        "Start small and scale up — there's no fixed minimum purchase.",
        "Every transaction is recorded on the ledger — verify your history any time from Transactions.",
      ],
      ctaHeading: "Ready to get started?",
      ctaBody: "Create an account in a few minutes and buy your first gold today.",
    },
  },
} as const;

export type Locale = keyof typeof dictionary;
// Union (not "bn"'s literal shape) — both locales must be structurally identical,
// but their string literal values differ, so the type has to admit either.
export type Dictionary = (typeof dictionary)[Locale];

export default dictionary;
