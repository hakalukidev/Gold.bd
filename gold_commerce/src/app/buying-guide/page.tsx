      ...
      <div className="bg-background py-16 sm:py-20">
        <div className="mx-auto max-w-3xl px-4 text-center sm:px-6">
          <h1 className="text-3xl font-bold sm:text-4xl">
            <GoldTitle text={g.heading} />
          </h1>
          <p className="mt-3 text-neutral-600 dark:text-neutral-300">{g.subheading}</p>
        </div>
        ...
        <div className="mx-auto mt-12 max-w-3xl px-4 sm:px-6">
          <ol className="flex flex-col gap-4">
            {g.steps.map((step, i) => (
              <li
                key={step.title}
                className="flex gap-4 rounded-md border border-black/10 bg-black/5 p-5 sm:p-6 dark:border-white/10 dark:bg-white/5"
              >
        ...
        <div className="mx-auto mt-10 max-w-3xl px-4 sm:px-6">
          <div className="rounded-md border border-black/10 bg-black/5 p-5 sm:p-6 dark:border-white/10 dark:bg-white/5">
            <p className="font-semibold text-neutral-900 dark:text-white">{g.tipsHeading}</p>
            <ul className="mt-3 flex flex-col gap-2">
              {g.tips.map((tip) => (
                <li key={tip} className="flex gap-2 text-sm text-neutral-600 dark:text-neutral-400">
        ...
        <div className="mx-auto mt-12 flex max-w-3xl flex-col items-center gap-4 px-4 text-center sm:px-6">
          <h2 className="text-xl font-semibold text-neutral-900 sm:text-2xl dark:text-white">{g.ctaHeading}</h2>
          <p className="max-w-md text-sm text-neutral-600 dark:text-neutral-400">{g.ctaBody}</p>
